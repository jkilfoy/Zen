...
| # | Step | Expected | Report |
|---|---|---|---|
| **P-1** | In an ordinary PowerShell, pick a directory **outside this repository** — `C:\Users\Jordan\keys\` will do — and run:<br><br>`keytool -genkeypair -v -keystore C:\Users\Jordan\keys\zen-release.jks -storetype JKS -keyalg RSA -keysize 4096 -validity 10000 -alias zen`<br><br>`keytool` comes with the JDK; if it is not on PATH, it is under the Android Studio JBR, e.g. `"$env:LOCALAPPDATA\Programs\Android Studio\jbr\bin\keytool.exe"`. It prompts for a store password, a key password, and then six certificate name fields. **Use the same password for both password prompts** unless you have a reason not to — two differing passwords is one more thing to lose.<br><br>The six name fields (CN, OU, O, L, ST, C) build the certificate's subject name. They have **no effect on whether upgrades work** — Android compares the key, not the name — but they ship inside every APK and cannot be changed later without a new key. Press Enter to leave any of them `Unknown`; that is a valid answer. `CN=Zen` and `O=dev.zen` with the rest blank is fine. Do not put a home address in: it is readable by anyone who runs `apksigner verify --print-certs`.<br><br>To skip the six prompts, add `-dname "CN=Zen, O=dev.zen"` — the passwords are still prompted for, which is where they belong. | `zen-release.jks` exists. 10000 days is ~27 years; a shorter validity would strand you at expiry with no way to upgrade. | That it was created, and **nothing about the passwords**. |
