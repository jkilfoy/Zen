...
| V2-15 | **Exercise sync against a real folder-sync client.** The convergence simulation uses a plain local directory, which is what Syncthing or a cloud drive presents but not how one behaves: partial files mid-replication, conflict copies, and a peer's file arriving before its contents are all real and none is tested. | M8 review; DECISIONS.md known limitations |
| V2-16 | **Goldens on the Linux CI runner.** They are generated on and selected by host platform, so CI skips them and a visual regression to the TODO-3 circle states is caught only by a local `verify.ps1`. Now unblocked: CI has run. Generate a Linux set with `--update-goldens` and select by platform. | D-M4-8; M8 review |
| V2-17 | **Re-measure `PRAGMA integrity_check` as the database grows.** It runs on every open (D-M3-11, §11.5.5) and is a full page scan. Negligible today — the whole of `main()` is 42 ms — but it scales with the file and this app is meant to run for years. Measure before optimising. | D-M3-11, D-M4-17; M8 review |
| V2-18 | **Prevent LAN replay rather than bounding it.** The five-minute skew check bounds a replay's window; it does not stop one. The cost is bounded and nothing tests it, because nothing currently stops it. A nonce cache or a challenge would close it. | Spec v1.14, §11.6.4; M8 review |
| V2-19 | **App icons are still Flutter's defaults** on both platforms — `ic_launcher.png` and `app_icon.ico` as generated by `flutter create`. Cosmetic, invisible to every test, and noticed while packaging: the installer and the Start menu entry both carry it. Deliberately not fixed in M8, which is not the milestone for inventing product surface. | M8 packaging review |
