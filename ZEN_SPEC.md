...
| Changes in 1.15 | Six findings from the M8 agent's pre-implementation review of §11.9, all accepted, plus one scope correction. **§11.9 gains the identity rules that make "every release installs over the last" true rather than hoped for:** the Inno Setup `AppId` GUID is the Windows counterpart to the Android keystore and must never change; `versionCode` must never decrease, which is also why the APK is a single universal binary rather than split per ABI; and the release build must **fail** when the keystore is absent instead of falling back to debug keys, since a debug-signed APK installs cleanly on the developer's own device and can then never be upgraded. **Two ways to destroy the data by editing something harmless-looking are now written down:** `CompanyName` and `ProductName` in `Runner.rc` decide where `%APPDATA%` holds the database, and the uninstaller must not remove that directory. **The Visual C++ runtime is not in the Flutter bundle**, so the installer ships it app-local; without it the application fails to launch on exactly the clean machine M8 is defined against. The install is per-user, so an unsigned binary adds no UAC prompt to its SmartScreen warning. **§11.10: CI does not build releases.** V1 ships from a local build; the tagged-release job is `V2-1` in `V2_BACKLOG.md`. M8's row in §11.13 says so too. |
...
**Versioning.** One number governs everything: `version:` in `zen_app/pubspec.yaml`. Its `x.y.z` part is the installer version, the APK `versionName`, and what SET-3's About row shows; the integer after `+` is the APK `versionCode`. **`versionCode` must never decrease.** Android refuses a lower one outright (`INSTALL_FAILED_VERSION_DOWNGRADE`), and the only way past that refusal is to uninstall, which destroys the local database. For the same reason the APK is a **single universal binary**: `--split-per-abi` adds `1000 × ABI index` to `versionCode`, so mixing a split build with a universal one produces a number that a later release cannot get above.
...
- **Windows:** the installer's **`AppId` GUID is the product's identity and must never change.** Inno Setup keys the upgrade-in-place path on it: a new `AppId` is a different product, and installing it leaves two entries in Add/Remove Programs and two copies on disk rather than upgrading the first. This is the Windows counterpart to the Android keystore below. It is recoverable — uninstall the stray — but it is the same class of mistake, so the GUID is a literal in the `.iss` with a comment saying so, never regenerated by a tool.
- **Windows:** `CompanyName` and `ProductName` in `windows/runner/Runner.rc` **decide where the database lives** and must not be edited. `path_provider_windows` builds `getApplicationSupportDirectory()` as `%APPDATA%\{CompanyName}\{ProductName}` — today `%APPDATA%\dev.zen\Zen`, holding `zen.sqlite`, the backups of §11.6.6 and the LAN pairing keys. Changing either string, which is a natural thing to do while polishing installer metadata, does not move that data: the application simply starts empty and the real database stays on disk where nothing looks for it.
- **Windows:** **the uninstaller must not remove the database.** Inno removes only what it installed, and the data directory above is not installed, so the correct behaviour is the default — but it is the default rather than a decision, and one `[UninstallDelete]` entry added to "clean up properly" would destroy every Idea and Task. The `.iss` says so where that entry would go.
- **Windows:** the Flutter release bundle does **not** contain the Visual C++ runtime, and a machine without the VC++ 2015–2022 redistributable fails to launch with a missing-DLL dialog that names nothing useful. The developer machine always has it, because the build toolchain installs it, so this failure appears only on the clean machine M8 is defined against. The installer therefore ships `msvcp140.dll`, `vcruntime140.dll` and `vcruntime140_1.dll` beside the executable, which Microsoft permits as app-local deployment. Chaining `VC_redist.x64.exe` is the alternative and is rejected: it adds ~25 MB and needs elevation, which would end the per-user install.
- **Windows:** the install is **per-user** (`PrivilegesRequired=lowest`), into `%LOCALAPPDATA%\Programs`. There is no UAC prompt, which matters for an unsigned binary that already draws a SmartScreen warning, and the data it manages is per-user anyway — a machine-wide install would imply a sharing story the application does not have.
...
- **Android:** the release build **must fail rather than fall back to the debug keystore** when the keystore is absent. A silent fallback produces an APK that installs perfectly on the developer's own device — the debug key is already trusted there — and can never be upgraded by a properly signed one. The failure must be loud and at build time, naming the missing file.
...
**The rule this all serves: every release installs over the last, and nothing is ever uninstalled.** An install that demands an uninstall first is not an inconvenience; it is the signal that the signing identity changed, and it must be investigated rather than worked around.
...
**CI does not build releases.** V1 ships from a local build: `tools/package.ps1` on the owner's Windows machine, which is the only machine with the Inno Setup toolchain and the signing keystore, and the only place the artifacts can be smoke-tested before they go anywhere. A tagged-release job — a Windows runner, Inno Setup, and the keystore as an encrypted secret — is **`V2-1` in `V2_BACKLOG.md`**, not M8. It is the right thing to build once there is a second machine or a second person; with neither, it adds a way for a release to be wrong that the local path does not have.
...
| **M8** | Packaging: Inno Setup installer, signed release APK, a single local build command, and the documentation a reader needs to repeat all of it. **Not the tagged-release workflow** — that is `V2-1` (§11.10) | **Requires a Windows machine and an Android device** (§11.13.1). Both artifacts install and run on clean machines, and the APK upgrades the previous one in place |
...
| M4–M5 widget and golden tests (`flutter test` is headless) | **Launching the app** on the owner's Android device, which runs **Android 13 (API 33)** — so anything gated on a newer API level, including the local-network behaviour that arrives at `targetSdk` 37 (§11.6.4), is never exercised by manual verification and must be reasoned about rather than observed. Also on Windows — M4's "runs on both platforms" is not established by widget tests, and this is the first milestone where that applies |
