...
* §11.6.5’s "on app foreground" was implemented as every `AppLifecycleState.resumed`. On Windows that fires when the window regains **focus**, so every click back into the app ran a full pass. A genuine departure from the foreground passes through `hidden`, `paused` or `detached` on both platforms — minimising on Windows, backgrounding on Android — so the scheduler now requires one of those before it treats a `resumed` as a foreground. `inactive` alone is a focus flicker, a menu or a dialog, and the app never stopped being on screen.
* The periodic timer was re-armed on **every** settings emission, and §11.6.5 step 8 writes `lastSyncAt` after every pass. So each sync reset the 15-minute timer and the interval never governed anything. It is now re-armed only when the interval value itself changes, which is all SET-2 asked for.

The first defect is why the second was invisible: passes came often enough from the lifecycle that the timer was always being reset before it could fire. Each now has its own regression test, and the one that asserted a bare `resumed` should sync was itself encoding the bug.

§11.6.5 says only "on app foreground", which is unambiguous on a phone and not on a desktop. This is the simplest reading consistent with it (§11.13’s standing instructions), but it is a candidate for a spec line if the owner wants the desktop meaning written down.
